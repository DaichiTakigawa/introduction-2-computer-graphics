...
import {get_camera, get_legacygl, get_drawutil, glu} from 'legacygl';

type Camera = ReturnType<typeof get_camera>;
type LegacyGL = ReturnType<typeof get_legacygl>;
type Drawutil = ReturnType<typeof get_drawutil>;
...
var legacygl: MyLegacyGL;
var drawutil: Drawutil;
var camera: Camera;
var p0: vec2, p1: vec2, p2: vec2;
var selected: vec2 | null = null;

declare global {
  interface GlobalEventHandlers {
    get_mousepos(event: MouseEvent, flip_y?: any): [number, number];
  }
...
function init() {
  ...
  canvas.onmousedown = function (evt) {
    var mouse_win = this.get_mousepos(evt) as number[];
    if (evt.altKey) {
      camera.start_moving(mouse_win, evt.shiftKey ? 'zoom' : 'pan');
      return;
    }
    // pick nearest object
    var points: vec2[] = [p0, p1, p2];
  ...
  canvas.onmousemove = function (evt) {
    ...
    if (selected != null) {
      ...
      var eye_to_mouse = vec3.sub([0, 0, 0], mouse_obj as vec3, camera.eye);
      var eye_to_origin = vec3.sub([0, 0, 0], plane_origin, camera.eye);
      var s1 = vec3.dot(eye_to_mouse, plane_normal);
      var s2 = vec3.dot(eye_to_origin, plane_normal);
      var eye_to_intersection = vec3.scale([0, 0, 0], eye_to_mouse, s2 / s1);
